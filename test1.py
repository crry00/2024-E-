...
class Chess:
    def __init__(self):
        ...
        self.white_hsv = [(89, 0, 0), (179, 81, 211)]
    ...
    def update(self):
        ...
        # mask = cv2.inRange(self.hsv, np.array(self.box_hsv[0]), np.array(self.box_hsv[1]))
        # cv2.erode(mask, mask, None, iterations=1)
        # SHOW("hsvmask", mask)
    ...
    #         self.points.sort(key=lambda point: np.linalg.norm(np.array(point) - np.array(center)))
    #         if np.linalg.norm(np.array(self.points[0]) - np.array(center)) > 50:
    #             return False
    #         updated_boxes.append(Box(self.points[0], box.get_status()))
    ...
    #         if sum(1 for b in new_boxes if np.linalg.norm(np.array(b.get_center()) - np.array(box.get_center())) < 10) >= 2:
    #             LOG("track() lost!")
    #             return -1
